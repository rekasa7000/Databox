...
namespace Databox
{
    class StudentAppointmentScheduler
    {
        ...
        public StudentAppointmentScheduler(Form form)
        {
            activeForm = form;
        }
        ...
        void getEmail()
        {
            ...
            db.OpenConnection();
            string query = $"SELECT email FROM user JOIN student ON user.user_id = student.user_id WHERE student_id = {sessionData.StudentId}";

            using (MySqlCommand cmd = new MySqlCommand(query, db.connection))
            {
                cmd.Parameters.AddWithValue("@student_id", sessionData.TeacherId);
        ...
        private void FetchSchedules()
        {
            ...
            Database_Connection db = new Database_Connection();
            db.OpenConnection();
            string query = "SELECT schedule.schedule_id, CONCAT(teacher.teacherFirstname, ' ', teacher.teacherLastname) AS classSelected, schedule.schedName, schedule.frequency, schedule.startTime, schedule.endTime, schedule.dayOfWeek, schedule.dayOfMonth, schedule.monthOfYear, schedule.scheduleDate " +
                "FROM schedule " +
                "JOIN teacher ON teacher.teacher_id = schedule.teacher_id " +
                "WHERE schedule.student_id = @studentId AND isActive = 1";

            using (MySqlCommand cmd = new MySqlCommand(query, db.connection))
            {
                cmd.Parameters.AddWithValue("@studentId", sessionData.StudentId);
        ...
        {
            string rem = $"Appointment Schedule - {classSelected}";
