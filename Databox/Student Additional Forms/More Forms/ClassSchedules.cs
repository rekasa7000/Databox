...
{
    public partial class ClassSchedules : Form
    {

        int student_id, totalClasses, currentIndex;
        Panel desktopPanel;
        public ClassSchedules(int student_id, Panel desktopPan)
        {
        ...
        private void ClassSchedules_Load(object sender, EventArgs e)
        {
            try
            {
                classLoad(0);
...
WHERE student.student_id = @studentID AND schedule.isActive = 1;
";
        ...
        {
            try { 
                StudentSchedules view = new StudentSchedules(student_id, desktopPanel);
                view.TopLevel = false;
