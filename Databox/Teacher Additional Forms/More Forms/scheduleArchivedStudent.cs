...
{
    public partial class scheduleArchivedStudent : Form
    {
        int teacher_id, currentIndex, totalClasses;
        Panel desktopPanel;
        public scheduleArchivedStudent(int teacherID, Panel pan)
        {
            desktopPanel = pan;
        ...
        void CalculateTotalClasses()
        {
            ...
            db.OpenConnection();
            using (MySqlCommand countCmd = new MySqlCommand("SELECT COUNT(*) FROM schedule WHERE teacher_id = @teacher_id AND isActive = 0 AND student_id != 0", db.connection))
            {
        ...
        private void scheduleArchivedStudent_Load(object sender, EventArgs e)
        {
            classLoad(0);
        }
        ...
        {
            ScheduleArchive view = new ScheduleArchive(teacher_id, desktopPanel);
            view.TopLevel = false;
